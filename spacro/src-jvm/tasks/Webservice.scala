                  ...
                  taskSpecOpt.flatMap { taskSpec =>
                    scala
                      ...
                      .Try {
                        ...
                        HttpEntity(
                          ContentTypes.`text/html(UTF-8)`,
                          printer.print(response.asJson(responseEncoder))
                        )
                      }
                      .toOption
                  }
    ...
    Flow[Message]
      .map {
        case TextMessage.Strict(msg) =>
          Future
            .successful(List(decode[HeartbeatingWebSocketMessage[WebsocketRequest]](msg).right.get))
        case TextMessage.Streamed(stream) =>
          stream                          // necessary to handle large messages
            ...
            .flatMap(msg =>
              Future.successful(
                List(decode[HeartbeatingWebSocketMessage[WebsocketRequest]](msg).right.get)
              )
            )
      ...
      .mapConcat(identity)
      .collect { case WebSocketMessage(request) =>
        request
      }                      // ignore heartbeats
      ...
      .map(message => TextMessage.Strict(printer.print(message.asJson)))
  }
}

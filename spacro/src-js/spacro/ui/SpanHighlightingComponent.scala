...
class SpanHighlightingComponent[Index] {
  ...
  @Lenses case class Highlighting(index: Index, anchor: Int, endpoint: Int)
      extends SpanHighlightingStatus
  ...
  class SpanHighlightingBackend(scope: BackendScope[SpanHighlightingProps, SpanHighlightingState]) {
    ...
    private[this] def modStateWithUpdate(
      f: SpanHighlightingState => SpanHighlightingState
    ): Callback =
      scope.props >>= { props =>
    ...
    def touch(props: SpanHighlightingProps)(index: Index)(wordIndex: Int): Callback =
      modStateWithUpdate {
        case SpanHighlightingState(spans, NoSpan) =>
        ...
        case SpanHighlightingState(spans, Highlighting(`index`, x, y)) =>
          ...
          if (relevantSpans.exists(_.contains(wordIndex)))
            SpanHighlightingState(spans, Highlighting(index, x, y)) // do nothing
          else
            SpanHighlightingState(spans.updated(index, Span(x, y) :: spans(index)), NoSpan) // finish span
        case x => x
      }
  ...
  val SpanHighlighting = ScalaComponent
    .builder[SpanHighlightingProps]("Span Highlighting")

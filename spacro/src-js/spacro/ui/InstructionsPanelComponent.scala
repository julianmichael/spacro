...
object InstructionsComponent {

  val dataToggle = VdomAttr("data-toggle")

  case class InstructionsProps(
    instructionsId: String,
    collapseCookieId: String,
    tabs: List[(String, VdomTag)]
  )

  class InstructionsBackend(scope: BackendScope[InstructionsProps, Unit]) {

    def render(props: InstructionsProps) =
      <.div(
        ^.classSet1("card"),
        <.div(
  ...
  val Instructions = ScalaComponent.builder[InstructionsProps]("Instructions")
    .renderBackend[InstructionsBackend]
    .componentDidMount(
    context => Callback {
      import scala.scalajs.js.Dynamic
      import org.scalajs.jquery.jQuery
      val cookieOpt = Dynamic.global.$.cookie(context.props.collapseCookieId).asInstanceOf[scalajs.js.UndefOr[String]].toOption
      ...
      Dynamic.global.$(s"#${context.props.instructionsId}").on(
        "hide.bs.collapse", () => {
          jQuery(s"#collapse-text").text("(Click to expand)")

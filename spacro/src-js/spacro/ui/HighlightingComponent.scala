...
class HighlightingComponent[Index] {
  ...
  @Lenses case class HighlightingState(span: Set[Index], status: HighlightingStatus)
  ...
  class HighlightingBackend(scope: BackendScope[HighlightingProps, HighlightingState]) {
    ...
    def render(props: HighlightingProps, state: HighlightingState) =
      props.render(
        state,
        HighlightingContext(setSpan, startHighlight, startErase, stop, touchElement(props))
      )
  }

  val Highlighting = ScalaComponent
    .builder[HighlightingProps]("Highlighting")

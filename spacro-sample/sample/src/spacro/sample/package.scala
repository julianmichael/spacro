...
package object sample {
  ...
  object SampleAjaxRequest {
    ...
    implicit val sampleAjaxRequestDotDecoder =
      new DotKleisli[Decoder, SampleAjaxRequest] {
        def apply(request: SampleAjaxRequest) = implicitly[Decoder[request.Out]]
      }
    implicit val sampleAjaxRequestDotEncoder =
      new DotKleisli[Encoder, SampleAjaxRequest] {
